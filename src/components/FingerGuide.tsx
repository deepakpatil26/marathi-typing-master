...
export const FingerGuide: React.FC<FingerGuideProps> = ({ activeFinger, language }) => {
  ...
  return (
    <div id="finger-guide-container" className={`w-full rounded-3xl p-5 shadow-xl backdrop-blur-md border transition-colors ${
      isDark 
      ...
      <div className={`flex flex-col sm:flex-row items-center justify-between gap-3 mb-4 pb-3 border-b ${
        ...
        {activeColorInfo && (
          <div className={`px-3.5 py-1 rounded-xl text-xs font-semibold border ${activeColorInfo.bg} flex items-center gap-2 transition-all duration-300 shadow-xs`}>
            <span className={isDark ? 'text-slate-300' : 'text-slate-700'}>
              {language === 'mr' ? 'वापरायचे बोट:' : 'Active Finger:'}
            </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className={`p-4 rounded-2xl border transition-all duration-200 ${
          isLeft || isThumb 
            ? isDark
              ...
              : 'bg-teal-50/70 border-teal-300 shadow-md shadow-teal-500/10'
            : isDark
          ...
          <div className="text-xs font-semibold mb-3 flex items-center justify-between">
            <span className={`font-bold ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>
              {language === 'mr' ? 'डावा हात (Left Hand)' : 'Left Hand'}
            </span>
            <span className={`text-[10px] font-mono ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              Home: a s d f (ो े ् ि)
            </span>
          </div>
              ...
              return (
                <div
                  ...
                  className={`flex flex-col items-center justify-center p-2.5 rounded-xl border text-center transition-all duration-200 ${
                    isActive
                      ? `${color.bg} ${color.border} scale-105 shadow-lg ring-2 ring-teal-400`
                      : isDark
                ...
                >
                  ...
                  <span className="text-[10px] opacity-75 mt-1 font-mono font-medium">{f.keyHint}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className={`p-4 rounded-2xl border transition-all duration-200 ${
          ...
          <div className="text-xs font-semibold mb-3 flex items-center justify-between">
            <span className={`font-bold ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>
              {language === 'mr' ? 'उजवा हात (Right Hand)' : 'Right Hand'}
            </span>
            <span className={`text-[10px] font-mono ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              Home: j k l ; (र ा स य)
            </span>
      ...
      <div className={`mt-4 pt-3 border-t flex items-center justify-center gap-2 text-xs ${
        ...
        <span className="font-medium">{language === 'mr' ? 'नेहमी शब्दांमधील अंतरासाठी स्पेसबार दाबा.' : 'Use thumb for pressing the Spacebar.'}</span>
      </div>
    </div>
  );
};
