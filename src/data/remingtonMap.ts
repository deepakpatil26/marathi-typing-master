  ...
  [
    {
      ...
      normalChar: '्र',
      shiftChar: 'र्',
      normalNameMr: 'र-प्रकार / पदस्थ र (्र)',
      shiftNameMr: 'रेफ (र्)',
      finger: 'left-pinky',
      hand: 'left',
      row: 'lower',
...
export function checkDevanagariMatch(
  ...
  for (
    let len = Math.min(4, targetText.length - currentIndex);
    len >= 1;
    len--
  ) {
  ...
  if (
    targetSlice2 === '\u093E\u0947' &&
    (typedNFC === '\u094B' || typedNFC === 'ो')
  ) {
    return {
      isMatch: true,
      advanceCount: 2,
      matchedLength: 2,
      expectedToken: targetSlice2,
    };
  }
  if (
    targetSlice2 === '\u093E\u0948' &&
    (typedNFC === '\u094C' || typedNFC === 'ौ')
  ) {
    return {
      isMatch: true,
      advanceCount: 2,
  ...
  if (
    (expectedChar === 'ो' || expectedChar === '\u094B') &&
    (typedNFC === '\u093E\u0947' || typedNFC === 'ाे')
  ) {
    return {
      isMatch: true,
      advanceCount: 1,
  ...
  if (
    (expectedChar === 'ौ' || expectedChar === '\u094C') &&
    (typedNFC === '\u093E\u0948' || typedNFC === 'ाै')
  ) {
    return {
      isMatch: true,
      advanceCount: 1,
  ...
  if (
    DEV_CHAR_ALIAS[expectedChar] === typedNFC ||
    DEV_CHAR_ALIAS[typedNFC] === expectedChar
  ) {
    return {
      isMatch: true,
      advanceCount: 1,
  ...
  const isMatch =
    expectedChar.normalize('NFC') === typedNFC || expectedChar === typed;
